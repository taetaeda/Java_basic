...
public class ArrayListExample {
	...
	public static void main(String[] args) {
        ...
        */
		...
		List<String> nick = new ArrayList</*String 생략 가능*/>(); 
		// <> : 제네릭  (타입의 형식을 지정하여 타입 안정성을 꾀하기 위한 문법)
		// <>를 안쓴다면 object 라서 다 받을 수 있다.
		...
		nick.add(str);
		nick.add("성삼경");
		nick.add(new String("유훈모"));
		nick.add("김한솔");
		nick.add("강희수");
		System.out.println(nick);
		
		//add(인덱스,객체) : 특정 인뎃그에 객체를 삽입하는 메서드
		
		nick.add(3,"정재현");
		System.out.println(nick);
		
		//get(인덱스) : 리스트 내부의 객체를 참조하는 메서드
//		String name = nick[3]; (x)배열의 방법이다
		...
		//contains(객체) : 리스트 내의 객체의 존재 유무를 파악하는 메서드 (boolean 타입)
		System.out.println(nick.contains("성삼경"));
		System.out.println(nick.contains("원정욱"));
		
		//indexOf : 특정 객체의 인덱스 탐색
		int idx = nick.indexOf("이경민");
		System.out.println("이경민의 인덱스: " + idx);
		...
		*/
		...
		//객체들을 모아두는 타입이기 떄문에 정수는 받을 수 없다. Integer 물어보기!!!!!!!!!!
		score.add(65);
		System.out.println(score);
		
		//한 번에 객체 많이 추가하기
		Collections.addAll(score, 66,67,68,69,70,71);
		System.out.println(score);
		
		//컬렉션에서의 등장 횟수 구하기
		System.out.println("70점 학생 수 : " + Collections.frequency(score, 70));
