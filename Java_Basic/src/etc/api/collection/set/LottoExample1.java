...
//    //보너스 번호를 생성하는 메서드
       public static int createBonusNum(Set<Integer> bn) {
...
//         */
    	int count = 0;
    	for(int n :myLotto) { // myLotto에 카운트가 6이니 6개가 다 맞을 떄까지 돌아간다.
    		if(win.contains(n)) count++;
    	}
    	
    	if(count == 6) prize1++; // 제일 위에 지정해뒀다. prize1~5,failCnt
    	else if(count == 5) {
...
//		//	로또 번호 생성 메서드를 호출해서 당첨 번호를 하나 고정시키세요.
    	Set<Integer> win = createLotto();  // 고정 win에 시키기
//       // 보너스번호도 하나 고정시키세요.
...
//             */
    	   Set<Integer> myLotto =  createLotto(); // 로또 1장 
    	   paper++; cost += 1000; // 로또 1장 가격
